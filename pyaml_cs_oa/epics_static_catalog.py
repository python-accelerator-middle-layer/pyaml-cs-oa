...
class EpicsStaticCatalog(Catalog):
    def __init__(self, cfg: ConfigModel):
        super().__init__(cfg)
        if not cfg.entries:
            raise PyAMLException("EpicsStaticCatalog.entries must contain at least one entry")
        self._refs: dict[str, DeviceAccess] = {}
        for entry in cfg.entries:
            key = entry.get_key()
            if key in self._refs:
                raise PyAMLException(f"EpicsStaticCatalog '{self.get_name()}': duplicate key '{key}'")
