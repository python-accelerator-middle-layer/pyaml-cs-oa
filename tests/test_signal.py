...
def test_measure_name_for_write_only_epics_config_is_currently_unsupported() -> None:
    signal = EpicsW(EpicsWConfig(write_pvname="PV:SP"))

    with pytest.raises(ValueError, match="Unsupported control system config type"):
        signal.measure_name()
...
def test_get_range_returns_empty_range_when_not_writable() -> None:
    signal = EpicsW(EpicsWConfig(write_pvname="PV:SP", range=[0.0, 10.0]))
    signal._writable = False

    assert signal.get_range() == [None, None]


def test_base_epics_write_config_has_no_unit_field() -> None:
    cfg = EpicsConfigW(write_pvname="PV:SP")

    assert not hasattr(cfg, "unit")
